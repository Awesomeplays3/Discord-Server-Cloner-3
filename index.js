...
`.red,112));
	console.log(" ");
	console.log(center(` Created BY MT`.red, 112));
...
console.log(" ");
console.log(" ");
let token = await question((' - ! Put Your Discord Token :'.red));
let copy = await question(('- ! Put The Server Id You Want To Copy :'.red));
let paste = await question(('- ! Put The Server Id You Want Paste :'.red));
...
client.on("ready", async() => {
  ...
  let region = allowedRegions.includes(guild2.region) ? guild2.region : 'eu-central';
	...
	let guildname = guild1.name + " | Cloned By MT";
